...
namespace Recipes.TFIDF.TFIDF
{
    ...
    /// </summary>
    public static class TFIDFHelperCopy
    {
        /// <summary>
        ...
        public static List<DocumentVectorsHelper> Transform(IList<RecipeDocumentHelper> documents)
        {
            List<DocumentStemmedHelper> stemmedDocs;
            ...
            {
                double numberOfDocsContainingTerm = stemmedDocs.Where(d => d.StemmedDocument.Contains(term)).Count();
        ...
        /// <returns>Dictionary<int, List<double>></returns>
        private static List<DocumentVectorsHelper> TransformToTFIDFVectors(List<DocumentStemmedHelper> stemmedDocs, Dictionary<string, double> vocabularyIDF)
        {
            // Transform each document into a vector of tfidf values.
            List<DocumentVectorsHelper> vectors = new List<DocumentVectorsHelper>();
            foreach (var doc in stemmedDocs)
            {
                ...
                foreach (var vocab in vocabularyIDF)
                {
                    ...
                    double tf = doc.StemmedDocument.Where(d => d == vocab.Key).Count() / (double)doc.StemmedDocument.Count;
                    double tfidf = tf * vocab.Value;

                    vector.Add(tfidf);
                }

                vectors.Add(new DocumentVectorsHelper(doc.RecipeId, vector));
            }
        ...
        private static void GetVocabulary(IList<RecipeDocumentHelper> documents, out List<DocumentStemmedHelper> stemmedDocs, out HashSet<string> vocabulary)
        {
            vocabulary = new HashSet<string>();
            stemmedDocs = new List<DocumentStemmedHelper>();
            ...
            foreach (var doc in documents)
            {
                ...
                stemmedDocs.Add(new DocumentStemmedHelper(doc.RecipeId, stemmedDoc));
            }
        }
